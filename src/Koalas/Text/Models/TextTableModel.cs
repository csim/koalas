                                   ...
                                   int? RowLimit) : IRender
{
    ...
    private void ComputeLayout()
    {
        ...
        {
            ComputeMeta();
            IEnumerable<int> dataColumnIndexes = from col in Columns
        ...
        {
            ComputeMeta();
            IEnumerable<int> dataRowIndexes = from row in Rows
                                              where row is DataTextRow
                                                    && !row.First
                                                    && Rows[row.Index - 1] is not IBorderTextRow
        ...
        {
            ComputeMeta();
            IEnumerable<int> insertIndexes = from row in Rows
                                             where row is HeadingTextRow
                                             from index in row.First
                                                               ? (row.Index + 1).Yield()
                                                               : row.Last
                                                                   ? row.Index.Yield()
                                                                   : [
        ...
        {
            IEnumerable<string> x = from row in Rows
